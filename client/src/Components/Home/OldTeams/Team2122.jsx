...
function Team2122(props) {
  ...
  return (
    <>
      <div >
        <h2 className="m-4 text-3xl text-center text-black">OUR STUDENTS TEAM (2021-2022)</h2>
        <div >
          {isLoading ? (<Loading size="80" width="10" speed="1" />) : (<>
            {team_21_22.map((items, index) => { return (<TeamCards key={`${index}` + items.post} index={index} item={items} />) })}
          </>)}
        </div>
      </div>
    </>
