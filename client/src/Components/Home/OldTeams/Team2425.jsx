                ...
                <>
                  <div className='flex flex-col items-center justify-center h-full rounded-2xl'
                    ...
                    >
                      ...
                      {/* <img src={imageLink}  alt={name} className="w-[50%] rounded-full text-center nav-darker-shadows align-center mx-auto mt-2 mb-2" referrerPolicy="no-referrer" /> */}
                      <img src={imageLink}  alt={name} className="w-[180px] h-[180px] object-cover rounded-full mx-auto mt-2 mb-2 nav-darker-shadows"  referrerPolicy="no-referrer"/>
...
function Team2425(props) {
  ...
  return (
    <>
      <div >
        <h2 className="m-4 text-3xl text-center text-black">OUR STUDENTS TEAM (2024-25)</h2>
        <div >
          {isLoading ? (<Loading size="80" width="10" speed="1" />) : (<>
            {team_24_25.map((items, index) => { return (<TeamCards key={`${index}` + items.post} index={index} item={items} />) })}
          </>)}
        </div>
      </div>
    </>
