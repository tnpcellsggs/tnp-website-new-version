...
function Team2526(props) {
  ...
  return (
    <>
      <div >
        <h2 className="m-4 text-3xl text-center text-black">OUR STUDENTS TEAM (2025-26)</h2>
        <div >
          {isLoading ? (<Loading size="80" width="10" speed="1" />) : (<>
            {team_25_26.map((items, index) => { return (<TeamCards key={`${index}` + items.post} index={index} item={items} />) })}
          </>)}
        </div>
      </div>
    </>
